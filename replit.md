...
- Game "Primeria" (monster-tamer RPG): `artifacts/mockup-sandbox/src/components/mockups/walk-demo/`
  ...
  - `save.ts` — localStorage save (key `primeria_v2`): `PartySave` + `WorldSave`. Exports `PartyMon = MonSpec & {level,xp}`; `PartySave.caught`/`box` are `PartyMon[]`. WorldSave includes: `wyvruntForm`, `wyrLoyalty`, `jayA3Wins`, `liaA3Wins`, `cleminusMet`, `demoComplete`. `PartySave.moves` stores active move IDs (max 4); migrated by `sanitizeActiveMoves` on load. Old saves (bare `MonSpec[]`) are hydrated by `hydrateParty()` in WalkDemo (level defaults to `wildLevelFor(rarity)`, xp 0).
