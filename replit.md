...
- **Area 4 = Deep Reaches** (south of Area 3). Background `area4-bg.png` (1280×896 source drawn into 1024×717 world) — coastal clifftop ruins: north yellow-path entrance, ocean cliff on the lower-right, ancient rune-archways/monoliths. ONLY connection is Area 3 ↔ Area 4 via the north yellow path: enter `A3_AREA4_EXIT` (in A3) → `A4_SPAWN={x:390,y:80}`; return via `A4_RETURN_A3=[330,0,450,35]` (north edge) → A3 (490,600). The old R2↔A4 doors were removed (east is now ocean cliff); Route 2 → Tidemark Shore is handled separately by `R2_SOUTH_BLOCKED`.
