...
- **Tidemark Shore** (south of Area 3, `scene="shore"`). Background `shore-bg.png` (1024×717 world) — coastal clifftop ruins: north yellow-path entrance, ocean cliff on the lower-right, ancient rune-archways/monoliths. ONLY connection is Area 3 ↔ Shore via the north yellow path: enter `A3_AREA4_EXIT` (in A3) → `A4_SPAWN={x:390,y:80}`; return via `A4_RETURN_A3=[330,0,450,35]` (north edge) → A3 (490,600). The old R2 south door is blocked by `R2_SOUTH_BLOCKED`.
